...
### 10.50. Chốt bù DUY NHẤT tháng 08 (tại sao tháng 08 không được lưu) — vẫn 100% tự động, KHÔNG có nút bấm tay (BẮT BUỘC deploy lại `send-due-reminders`, KHÔNG cần chạy SQL)
...
rất có thể lượt cron chạy sáng 31/08 đã diễn ra TRƯỚC KHI kịp deploy code mới lên Supabase, nên lượt chạy
đó vẫn dùng bản Edge Function CŨ (chưa có đoạn tự lưu số liệu) — không có gì được ghi. Điều kiện tự chốt
chỉ kiểm tra "đúng ngày cuối tháng" — **duy nhất 1 ngày trong cả tháng** — lỡ đúng ngày đó thì mất, không
tự bù. Cơ chế tự chốt cuối tháng cho các tháng SAU NÀY (09, 10, 11...) **giữ nguyên như cũ, không đổi gì**
— vẫn 100% tự động, không cần thao tác gì.

**Chỉ chốt bù đúng tháng 08** (không phải cơ chế tổng quát dò mọi tháng thiếu) — các tháng lịch sử trước
đó bạn sẽ tự nạp qua tính năng nạp Excel riêng sau này, không cần app tự đoán. Đoạn code chốt bù này chỉ
nhắm đúng `year_month = '2026-08'`, tự kiểm tra đã có dòng đó chưa trước khi chốt (vô hại nếu chạy lại
nhiều lần) — vẫn hoàn toàn tự động (chạy ở lượt cron hàng ngày kế tiếp sau khi deploy, không cần bấm gì).

**Về độ chính xác của tháng 08**: cách tính "Dư nợ theo nhóm nợ" cần dữ liệu hợp đồng TẠI ĐÚNG ngày 31/08,
nhưng hệ thống không lưu "ảnh chụp" hợp đồng theo từng ngày trong quá khứ, chỉ có dữ liệu HIỆN TẠI. Nếu từ
31/08 tới lúc deploy đã có phát sinh thu tiền/tất toán/giải ngân mới, số liệu tháng 08 chốt bù có thể lệch
so với đúng ngày 31/08 thật.

**Việc cần bạn làm**: deploy lại Edge Function `send-due-reminders` — Supabase Dashboard → Edge Functions
→ `send-due-reminders` → dán đè toàn bộ nội dung file `supabase/functions/send-due-reminders/index.ts`
mới nhất trong repo này → Deploy. KHÔNG cần chạy SQL. Deploy xong, lượt cron tự động kế tiếp (8h sáng giờ
VN hôm sau) sẽ tự chốt tháng 08 — không cần bấm gì.
